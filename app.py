...
def get_fused_data():
    ...
    # Subtract the mean and divide by the standard deviation for the viirs samples
...
def get_features(longitude, latitude):
    ...
    dataclean = gee_data.select(trainingbands).sampleRegions(collection= poi_geometry, properties=[label], scale=scaleFactor)  # Use getInfo to load the sample's features
